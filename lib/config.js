...
export const siteConfig = {
  ...
  author: {
    ...
    bio: 'Giorgi spent nine years behind the counter at a state DMV licensing office before opening his own small used-car lot. He writes the guides he wishes someone had handed him on day one.',
    avatar:
  ...
  // Google AdSense publisher ID (public — appears in the page HTML).
  adsenseClient: process.env.NEXT_PUBLIC_ADSENSE_CLIENT || 'ca-pub-1649318551620186',
  // Google Analytics (GA4) measurement ID. Not a secret — appears in page HTML.
  gaId: process.env.NEXT_PUBLIC_GA_ID || 'G-4P2QY3KKZC',
  // Set true once the property is verified in Google Search Console
  // (we verified via a DNS TXT record). Drives the admin integrations status.
  searchConsoleVerified: (process.env.NEXT_PUBLIC_GSC_VERIFIED ?? 'true') !== 'false',
  defaultOgImage:
