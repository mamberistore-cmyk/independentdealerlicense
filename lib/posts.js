...
export function getPosts() {
  ...
  // Public listings show published posts only (drafts/pending/private hidden),
  // and scheduled posts only once their date has arrived. Newest first.
  const now = Date.now();
  return posts
    .filter((p) => {
      if (p.status === 'published') return true;
      if (p.status === 'scheduled') return new Date(p.date).getTime() <= now;
      return false;
    })
...
export function getPostBySlug(slug) {
  ...
  return {
    ...
    cover: data.cover || fallbackCover(slug),
    author: data.author || null,
    // Optional SEO/social overrides written by the editor.
